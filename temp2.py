...
#try except for  saftey, wait on rate limit should handle that
...
def makeIt(target , following ,mentions , followers , hashtags):
    ...
    for user in mentions:
        mG.add_edge(target,user)
        if user not in account_list:
            account_list.append(user)
    for hashtag in hashtags:
        hG.add_edge(hashtag,target)

    temp_list=account_list
    temp_list.remove(target.replace("@",""))
...
"""
        following=str.split('\n')
        makeIt(target ,following , mentions , myfollower , hashtags)
